...
import { applyContentProtection } from './window-protection'
import {
  showToolbar,
  hideToolbar,
  setToolbarWanted,
...
let compactModeRestoreBounds: Rectangle | null = null
...
/** Keep restored bounds visible if the display layout changed while compact mode was active. */
function fitBoundsToVisibleWorkArea(bounds: Rectangle): Rectangle {
...
/**
 * Temporarily reduce the main window to a tiny, transparent-background answer overlay.
 * The renderer owns the visual treatment; main owns native bounds and restores
 * the exact previous window geometry when the mode is turned off.
 */
function toggleCompactMode() {
  ...
  if (!state.compactMode) {
    const restoreBounds = mainWindow.getBounds()
    compactModeRestoreBounds = restoreBounds
    const compactBounds = fitBoundsToVisibleWorkArea({
      ...restoreBounds,
      width: COMPACT_MODE_WIDTH,
      height: COMPACT_MODE_HEIGHT
    })
    state.compactMode = true
    hideToolbar()
    mainWindow.setBounds(compactBounds)
  } else {
    state.compactMode = false
    if (compactModeRestoreBounds) {
      mainWindow.setBounds(fitBoundsToVisibleWorkArea(compactModeRestoreBounds))
    }
    compactModeRestoreBounds = null
  }
...
ipcMain.handle(
  'initShortcuts',
  (_event, configuredShortcuts: Record<string, { action: string; key: string }>) => {
    Object.entries(configuredShortcuts).forEach(([action, { key }]) => {
...
ipcMain.handle('toggleCompactMode', () => {
  toggleCompactMode()
