...
const MIN_VISIBLE_CONTENT_OPACITY = 0.05
...
export function createToolbarWindow(parent: BrowserWindow): void {
  ...
  toolbarWindow.setOpacity(getToolbarContentOpacity(toolbarOpacity))
...
/** Exposes the native toolbar window only to the main-process compatibility check. */
export function getToolbarWindowForDiagnostics(): BrowserWindow | null {
...
export function setToolbarOpacity(opacity: number): void {
  ...
  if (!toolbarWindow || toolbarWindow.isDestroyed()) return
  toolbarWindow.setOpacity(getToolbarContentOpacity(opacity))
  toolbarWindow.webContents.send('sync-toolbar-opacity', {
    surfaceOpacity: getToolbarSurfaceOpacity(opacity)
  })
}

function getToolbarContentOpacity(opacity: number): number {
  return Math.max(MIN_VISIBLE_CONTENT_OPACITY, opacity)
}

function getToolbarSurfaceOpacity(opacity: number): number {
  return opacity / getToolbarContentOpacity(opacity)
}
