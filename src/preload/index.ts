...
import type { StealthCompatibilityReport } from '../main/stealth-check'
...
const api = {
  ...
  // Toggle the temporary compact presentation and return its resulting state
  toggleCompactMode: () => ipcRenderer.invoke('toggleCompactMode') as Promise<boolean>,
  ...
  onSyncToolbarOpacity: (callback: (settings: { surfaceOpacity: number }) => void) => {
    ipcRenderer.on('sync-toolbar-opacity', (_event, settings) => {
      callback(settings)
    })
  },
  removeSyncToolbarOpacityListener: () => {
    ipcRenderer.removeAllListeners('sync-toolbar-opacity')
  },
  ...
  // Ask the renderer-owned settings store to activate the next Prompt scene
  onCyclePromptScene: (callback: () => void) => {
    ipcRenderer.on('cycle-prompt-scene', callback)
  },
