...
import { getWindowOpacityLayers } from '@/lib/window-opacity'
...
export function OverlayToolbar() {
  ...
  useEffect(() => {
    window.api.getAppSettings().then((settings) => {
      ...
      applySurfaceOpacity(getWindowOpacityLayers(settings.opacity).surfaceOpacity)
    })
    window.api.onSyncToolbarSettings(({ hoverDelay }) => {
      setHoverDelay(hoverDelay || 0)
    })
    window.api.onSyncToolbarOpacity(({ surfaceOpacity }) => {
      applySurfaceOpacity(surfaceOpacity)
    })
    return () => {
      window.api.removeSyncToolbarSettingsListener()
      window.api.removeSyncToolbarOpacityListener()
    }
  }, [])
...
function applySurfaceOpacity(surfaceOpacity: number): void {
