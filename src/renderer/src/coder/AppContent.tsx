...
export function AppContent() {
  ...
  const showScreenshotPreview = useSettingsStore((state) => state.showScreenshotPreview)
  const screenshotDisplay = useSettingsStore((state) => state.screenshotDisplay)
  const answerTextColor = useSettingsStore((state) => state.answerTextColor)
  ...
  return (
    <div
      id="app-content"
      className="px-6 py-4"
      style={{ '--answer-text-color': answerTextColor } as CSSProperties}
    >
      {/* Error Banner */}
      {!compactMode && errorMessage && (
